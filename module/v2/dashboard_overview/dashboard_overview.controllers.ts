...
export const seallingLocationRevenue = async (req: Request, res: Response) => {
  try {
    ...
    const [insolesResult, shoesResult] = await Promise.all([
      // Insoles: Group by geschaeftsstandort (JSON display or backup legacyString), calculate revenue and count
      prisma.$queryRaw<
        Array<{ location: string; revenue: number; count: number }>
      >`
        SELECT 
          COALESCE("geschaeftsstandort"->>'display', "backupGeschaeftsstandort"->>'legacyString', 'Unknown') as location,
          COALESCE(SUM("totalPrice"), 0)::float as revenue,
          COUNT(*)::int as count
        FROM "customerOrders"
        WHERE "partnerId" = ${id}::text
          AND "orderStatus" = 'Ausgeführt'
          AND ("geschaeftsstandort" IS NOT NULL OR "backupGeschaeftsstandort" IS NOT NULL)
        GROUP BY COALESCE("geschaeftsstandort"->>'display', "backupGeschaeftsstandort"->>'legacyString', 'Unknown')
      `,
