...
export const getPickupByOrderId = async (req: Request, res: Response) => {
  try {
    ...
    const orderWhere =
      isAdmin || !partnerId ? { id: orderId } : { id: orderId, partnerId };
    ...
    if (!isAdmin && partnerId && order.partnerId !== partnerId) {
      return res
        .status(403)
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        status:
          o === "Ausgeführt"
            ? "Picked up"
            : ["Abholbereit_Versandt", "Verpacken_Qualitätssicherung"].includes(
                  o,
                )
              ? "Ready"
              : o.replace(/_/g, " "),
