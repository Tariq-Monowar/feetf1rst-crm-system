...
export const generateNextCustomShaftOrderNumber = async (
  partnerId: string,
): Promise<string> => {
  const result = await prisma.$queryRaw<Array<{ orderNumber: string }>>`
    SELECT "orderNumber"
...
export const getTotalPriceRatio = async (req: Request, res: Response) => {
  try {
    ...
    console.log(
      `Sample transitions (last 5):`,
      JSON.stringify(sampleTransitions, null, 2),
    );
    ...
    console.log(
      `Found ${transitions.length} transitions for partner ${id} in ${month}/${year}`,
    );
    if (transitions.length > 0) {
      console.log(
        `Sample filtered transitions:`,
        JSON.stringify(transitions.slice(0, 3), null, 2),
      );
    }
...
export const getAllTransitions = async (req: Request, res: Response) => {
  try {
    ...
    const conditions: Prisma.Sql[] = [
      Prisma.sql`aot."partnerId" = ${id}::text`,
    ];
    if (status)
      conditions.push(
        Prisma.sql`aot.status = ${status}::"admin_order_transitions_status"`,
      );
    if (orderFor)
      conditions.push(
        Prisma.sql`aot."orderFor" = ${orderFor}::"transitions_for"`,
      );
    ...
    const data = transitionsData.map((row) => {
      const custom_shafts = row.cs_id
        ? {
            ...
            deliveryDate: row.cs_deliveryDate,
            createdAt: row.cs_createdAt,
          }
        : null;
